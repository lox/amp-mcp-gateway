...
| Tool policies | Implemented | Browser-managed allow / require approval / deny; new/changed tools disabled in review. Unknown tools cannot execute. |
...
| Tool-definition review | Partial | Explicit fetch/review/save with schemas and expiring snapshots; no automatic drift detection or quarantine. |
...
- [ ] Allow policy revocation without fetching the provider's tool list again.
