...
<span class="badge {{if eq .Status "Healthy"}}succeeded{{else}}pending{{end}}">{{.Status}}</span>
<p class="help">{{.Detail}}</p>{{if .Refresh}}<p class="help">{{.Refresh}}</p>{{end}}
...
{{if not .RetryAt.IsZero}}<p>Retry after: {{.RetryAt.Format "02 Jan 2006 15:04:05 UTC"}}</p>{{end}}</div>{{end}}{{end}}
...
.connection-status{margin:16px 0}.connection-health .help{font-size:14px;margin:10px 0}.health-times{font-size:12px;color:#586b62}.health-times p{margin:5px 0}.connection-test.actions{margin-top:14px}.connection-test button:disabled{cursor:wait;opacity:.65}
...
{{else}}<h1>Connections and activity</h1><p class="sub">Choose the tools agents can use. Review calls that need your approval.</p><div class="heading"><h2>Connections</h2><a class="button primary" href="/connections/new">Add MCP</a></div><div class="grid">{{range .Connections}}<div class="card"><strong>{{.ID}}</strong><p class="sub">{{.Account}}</p>
...
document.querySelectorAll('.connection-test').forEach(form => form.addEventListener('submit', async event => {
  ...
  try {
    ...
    section.querySelector('.connection-health').innerHTML = await response.text();
  } catch (_) {
